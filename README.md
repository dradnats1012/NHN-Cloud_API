# NHN Cloud Infrastructure Provisioning Tool

NHN Cloud API를 활용한 클라우드 인프라 자동 프로비저닝 웹 도구입니다.
Streamlit 기반 UI를 통해 VPC, 서브넷, 키페어, 인스턴스를 생성하고 관리할 수 있습니다.

## 주요 기능

- **토큰 인증** — NHN Cloud Identity API를 통한 인증 토큰 발급
- **네트워크 관리** — VPC/서브넷 생성 및 CIDR 유효성 검증, 네트워크 리소스(VPC, 서브넷, 보안그룹) 조회
- **인스턴스 생성** — 이미지, 플레이버, 서브넷, 보안그룹, 키페어를 선택하여 인스턴스 생성
- **키페어 관리** — 새 키페어 생성 또는 기존 공개키 등록 (PEM 다운로드 지원)
- **스마트 필터링** — 이미지 최소 요구사항(RAM, Disk)에 맞는 플레이버만 표시

## 프로젝트 구조

```
nhn_cloud_api/
├── main.py                   # Streamlit 앱 엔트리포인트
├── nhncloud_common.py        # 공통 유틸 (에러 처리, 헤더 생성)
├── nhncloud_token.py         # 토큰 발급 스크립트
├── nhncloud_network_api.py   # 네트워크 API (VPC, 서브넷, 키페어 생성)
├── nhncloud_network_ui.py    # 인스턴스 생성 UI + API (통합 모듈)
└── pages/
    ├── Network.py            # 네트워크 페이지 (VPC/서브넷 생성 및 조회)
    └── Instance.py           # 인스턴스 페이지 (리소스 선택 → 인스턴스 생성)
```

## Quick Start

### 1. 클론 및 의존성 설치

```bash
git clone https://github.com/dradnats1012/NHN-Cloud_API.git
cd NHN-Cloud_API
pip install -r requirements.txt
```

### 2. 환경 변수 설정

프로젝트 루트에 `.env` 파일을 생성합니다.

```env
NHNCloud_tenantID=<your-tenant-id>
NHNCloud_userID=<your-username>
NHNCloud_password=<your-api-password>
```

### 3. 앱 실행

```bash
streamlit run nhn_cloud_api/main.py
```

## 사용 흐름

1. **토큰 발급** — Tenant ID, Username, API Password를 입력하여 인증 토큰 발급
2. **네트워크 구성** — Network 페이지에서 VPC/서브넷 생성 또는 기존 리소스 조회
3. **인스턴스 생성** — Instance 페이지에서 리소스(이미지, 플레이버, 서브넷 등)를 선택 후 인스턴스 생성

## 기술 스택

| 구분 | 기술 |
|------|------|
| 언어 | Python 3 |
| UI | Streamlit |
| HTTP | requests |
| 환경 변수 | python-dotenv |
| 대상 클라우드 | NHN Cloud (kr1 리전) |
