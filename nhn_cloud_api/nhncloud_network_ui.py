...
with st.form("auth"):
    ...
    st.subheader("엔드포인트")
    network_base = st.text_input("Network Endpoint", value=DEFAULT_NETWORK_BASE)
    compute_base = st.text_input("Instance Endpoint", value=DEFAULT_COMPUTE_BASE)
    image_base = st.text_input("Image Endpoint", value=DEFAULT_IMAGE_BASE)
...
if submitted_auth:
    try:
        ...
        st.session_state.token = issue_token(tenant_id.strip(), username.strip(), api_password)
        st.success("토큰 발급 성공")
...
if refresh or "resources" not in st.session_state:
    try:
        ...
        resources = {
            "vpcs": list_vpcs(network_base.strip(), token),
            "subnets": list_subnets(network_base.strip(), token),
            "security_groups": list_security_groups(network_base.strip(), token),
            "images": list_images(image_base.strip(), token),
            "flavors": list_flavors(compute_base.strip(), token, tenant_id.strip()),
            "keypairs": list_keypairs(compute_base.strip(), token, tenant_id.strip()),
        }
...
with colA:
    ...
    selected_vpc = st.selectbox("VPC (참고용)", options=vpc_labels or ["(없음)"], disabled=not bool(vpc_labels))
    selected_subnet = st.selectbox("Subnet", options=subnet_labels or ["(없음)"], disabled=not bool(subnet_labels))
...
if create_btn:
    try:
        ...
        server = create_instance(
            compute_base=compute_base.strip(),
            token=st.session_state.token,
